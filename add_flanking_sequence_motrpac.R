...
fasta_dir <- "//iron/proteomics_storage_slow/MoTrPAC/RefSeq/20181127_Rat_RefSeq_nonredundant_annotated_first/RefSeq.20181127_rat_full.fasta"
dat <- read_tsv("~/motrpac-pass/motrpac-bic-pass1b-data/proteomics/t58-heart/prot-ph/motrpac_pass1b-06_t58-heart_prot-ph_ratio-results.txt")

#Check for the ptm_id column in the data frame
if(!("ptm_id" %in% colnames(dat) )){
  stop("Column ptm_id not found in data frame")
}

#Read FASTA file
fasta <- readAAStringSet(fasta_dir)

#Extract refseq accession number from FASTA metadata
names(fasta) <- str_extract(names(fasta),"(X|N)P_\\d+\\.?\\d")


mapping_table <- 
  data.frame(ptm_id = dat$ptm_id) %>%
  
  #Extract protein_id and ptm_sites from mapping table
  mutate(protein_id = str_extract(ptm_id,"(X|N)P_\\d+\\.?\\d"),
         ptm_sites = str_extract_all(ptm_id,"[[:alnum:]]+$")) %>%
  
  #Remove proteins not found in FASTA file
  filter(ptm_id %in% names(fasta_dir)) %>%
  
  #Split ptm_sites into multiple rows and remove empty rows
  separate_rows(ptm_sites, sep = "[[:lower:]]") %>% 
  filter(ptm_sites != "") %>%
